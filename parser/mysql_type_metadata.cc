...
/*
std::pair<int64_t, uint64_t>
MySQLNewDateMetaData::supportsRange(const Create_field &field) const
{
    // 8 bytes
    const unsigned int bytes = 8;
    return supportsRangeHelper(field, bytes);
...
fetchInt(enum enum_field_types type)
{
  ...
  // if(MYSQL_TYPE_NEWDATE == type)
   
      const MySQLLongLongMetaData *const integer_meta = 
      static_cast<MySQLLongLongMetaData *>(abstract.get());
   /*
    else
    {   
...
/*
static const AbstractMySQLDateMetaData<MYSQL_TYPE_NEWDATE> &
fetchDate(enum enum_field_types type)
{
    ...
    const AbstractMySQLDateMetaData<MYSQL_TYPE_NEWDATE> *const date_meta =
        static_cast<AbstractMySQLDateMetaData *>(abstract.get());
